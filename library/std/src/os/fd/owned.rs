...
impl BorrowedFd<'_> {
    ...
    /// Creates a new `OwnedFd` instance that shares the same underlying file
    /// description as the existing `BorrowedFd` instance.
    #[cfg(any(target_arch = "wasm32", target_os = "hermit", target_os = "trusty", target_os = "badgevms"))]
    #[stable(feature = "io_safety", since = "1.63.0")]
    pub fn try_clone_to_owned(&self) -> io::Result<OwnedFd> {
...
impl Drop for OwnedFd {
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            #[cfg(not(any(target_os = "hermit", target_os = "badgevms")))]
            {
                #[cfg(all(unix, not(target_os = "badgevms")))]
            ...
            {
                let _ = why2025_badge_sys_bindings::close(self.fd.as_inner());
            }
